...
def prepare_ged_dataframe(df, config, strategy):
    ...
    for index, scan in df.iterrows():
        ...
        try:
            eigenvalues_file = os.path.join(spectrum_dir, os.path.basename(scan['preproc_filename_cifti']).replace('.dtseries.nii', '_ged_eigenvalues.txt'))
    ...
    for frequency in df_out['frequency'].unique():
        print(f'Calculating partial residuals for frequency: {frequency}',end='\r', flush=True)
        for eigenvector in range(config['num_eigenvectors_to_keep']):
            ...
            if df_stat.shape[0] != 125:
                raise ValueError(f'Expected 125 scans for frequency {frequency} and eigenvector {eigenvector} but got {df_stat.shape[0]}')

            df_stat = df_stat.rename(columns={'ratio_outliers_fd0.5_std_dvars1000': 'ratio_outliers_fd0_5_std_dvars1000'})

            formula0 = "eigenvalue ~ 1 + " + " + ".join(config['nuisance_regressors'])
            ...
            formula1 = "eigenvalue ~ 1 + " + " + ".join(config['nuisance_regressors_nomotion'])
...
if __name__ == "__main__":
    ...
    for strategy in denoising_strategies:
        ...
        spectrum_df.to_csv('data/results/ged_by_frequency_eigenvector_'+strategy+'.csv', index=False)